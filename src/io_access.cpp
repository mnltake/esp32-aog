...
#include "io_provider_fxl6408.hpp"
  ...
  gpio_num_t IoAccess::getRawIo(uint8_t port) {return provider[port / 32] -> getRawIo(port % 32);}

  // normal interactions during runtime
  bool IoAccess::getDigitalInput(uint8_t port) {return provider[port / 32] ->  getDigitalInput(port % 32);}
  int IoAccess::getAnalogInput(uint8_t port) {return provider[port / 32] ->  getAnalogInput(port % 32);}
  float IoAccess::getAnalogInputScaled(uint8_t port) {return provider[port / 32] ->  getAnalogInputScaled(port % 32);} // FSR scaled to a 0-1
  void IoAccess::setDigitalOutput(uint8_t port, bool state) {provider[port / 32] -> setDigitalOutput(port % 32, state);}
  void IoAccess::setPwmOutput(uint8_t port, uint8_t dutyCycle) {provider[port / 32] -> setPwmOutput(port % 32, dutyCycle);}
